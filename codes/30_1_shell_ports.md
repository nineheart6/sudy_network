...
if [ $(ss -tulnp | grep "$V_PORT1" | wc -l) = 1 ]; then
        echo "port $V_PORT1 = active"
else
        echo "port $V_PORT1 = deactive"
fi

if [ $(ss -tulnp | grep "$V_PORT3" | wc -l) = 1 ]; then
        echo "port $V_PORT3 = active"
else
        echo "port $V_PORT3 = deactive"
fi

if [ $(ss -tulnp | grep "$V_PORT2" | wc -l) = 1 ]; then
        echo "port $V_PORT2 = active"
else
        echo "port $V_PORT2 = deactive"
fi

[mk@192.168.0.100 ~/Documents]$ ./check_ports.sh 7030 7000 8000
port 7030 = deactive
port 8000 = deactive
port 7000 = active
...
V_PID=$(lsof -i:"$V_PORT" | tr -s " " | tail -n +2 | head -1 | cut -d" " -f2)
V_NAME=$(lsof -i:"$V_PORT" | tr -s " " | tail -n +2 | head -1 | cut -d" " -f1)

if [ "$V_PID" == "" ]; then
        echo "no process using $V_PORT"
else
        echo "pid = $V_PID command = $V_NAME"
        kill -9 "$V_PID"
fi


[mk@192.168.0.100 ~/Documents]$ ./kill_port.sh 8080
no process using 8080
[mk@192.168.0.100 ~/Documents]$ ./kill_port.sh 7000
pid = 10791 command = python3
[2]+  Killed                  nohup python3 -m http.server 7000 --bind 0.0.0.0  (wd: ~/Downloads/webroot)
(wd now: ~/Documents)
```
---
...
#"$V_SERVER1 $V_SERVER2"에서 telnet은 "google.com 80"이라는 한 개의 문자열을 호스트로 인식하고, 포트는 생략
timeout 3 telnet $V_SERVER1 $V_SERVER2  > test.log
